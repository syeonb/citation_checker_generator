...
- **Supplemental Reports**:
  ...
  - `result/duplicates.txt` - Human-readable report of duplicate/similar entries
  - `result/duplicates.json` - JSON format duplicate groups with similarity scores
...
Results are saved to `result/duplicates.txt` and `result/duplicates.json`.
...
- Duplicate detection uses configurable thresholds (see `DuplicateChecker` class in code). Lower thresholds will find more potential duplicates but may have false positives.
